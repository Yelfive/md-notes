...
4. 客服端发送 PMS 以及已发送报文的 MAC，这些报文是在加密算法协商过程中发送，所以未曾加密。
   1. PMS, 服务器可以使用 PMS 生成主秘钥 PM，然后切片生成 2 个对称加密秘钥、2 个报文鉴别秘钥。
...
2. $M_B$, Bob 的鉴别秘钥，用于保证发送给 Alice 的报文完整性。
3. $E_A$, Alice 的对称秘钥，用于加密发送给 Bob 的报文。
4. $M_A$, Alice 的鉴别秘钥，用于确保发送给 Bob 的报文完整性。

:::tip
四个秘钥 Alice 与 Bob 共享，都是通过 PMS 与不重数计算得到。这种从 PMS 中计算得到的方式，使得通信双方可以不直接传输秘钥，就可以达成一致的结果，进一步保证了安全性。
:::
...
**A.** 为了避免 “连接[重放攻击](https://zh.wikipedia.org/wiki/%E9%87%8D%E6%94%BE%E6%94%BB%E5%87%BB)”。假设你从淘宝购买了意见商品，并完成了扣款，而这个过程被第三者小黑监听了（比如你连了他家的 WiFi）。他虽然无法知道你购买商品的细节，但是（先假设没有不重数），他可以录制你的所有请求，然后第二天向淘宝重放请求。没有不重数时，你与淘宝之间将生成相同的 PMS，最后生成相同的加解密秘钥，所有的报文都能通过完整性检测。于是淘宝认为你是要购买相同的商品，于是又把支付宝给你扣了。图 2 展示了这个过程。
...
有了不重数，在 **握手阶段**，淘宝会生成一个与上次会话中不同的不重数发送给小黑，小黑应该基于这个不重数生成一个不同的 MS。而小黑重放时使用的却是通过原来的 MS 计算出的鉴别秘钥经过散列函数得到的 MAC 以及原来的 PMS。淘宝收到 PMS 后根据新的不重数生成 MS，切片得到新的鉴别秘钥，会意外地发现这个握手报文无法通过完成性校验。从而可以判断这是一条非法的请求并断开链接。

**Q. 握手的前两个阶段是明文的，小黑完全可以拿到服务器不重数之后自己计算出新的秘钥，然后伪造你的购物请求，达到攻击的目的，这个怎么解决？**

**A.** 小黑确实可以伪造一个请求，但是购物请求通常都带有你的身份鉴别 token。小黑无法得知录制的请求中包含的 token 等任何信息，也就无法伪造你发送的任何请求。他只能建立一个自己的 TLS 连接。
...
**A.** SSL 采用 TCP 类似的方案，添加序列号。为了方便讨论，我们假设发送方为 A，接收方为 B。A 与 B 同时维护一个 A 的序列号，每发送一个分组，就将序列号加一。当 A 发送数据给 B 时，将序列号加入散列函数，一同计算 MAC。B 收到分组时，将其维护的 A 的序列号加入散列函数计算 MAC，对比是否一致，从而防止重放攻击。即，

$$
MAC = H(message + M_A + SEQ)
$$
